...
def test_build_kafka_config_sets_plain_broker_defaults(kafka_module):
    conf = kafka_module.build_kafka_config({"brokers": "broker1:9092,broker2:9092"})
...
def test_build_kafka_config_sets_sasl_ssl(kafka_module, monkeypatch):
    monkeypatch.setattr(kafka_module, "_write_temp_file", lambda content: f"/tmp/{content}" if content else None)

    conf = kafka_module.build_kafka_config(
        {
